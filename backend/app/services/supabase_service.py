...
def get_supabase_client() -> Client:
    try:
        if settings.SUPABASE_PROJECT_URL and settings.SUPABASE_API_KEY:
            logger.debug(f"Initializing Supabase client with URL: {settings.SUPABASE_PROJECT_URL}")
            logger.debug(f"Using API Key (last 4 chars): ...{settings.SUPABASE_API_KEY[-4:]}")
            return create_client(settings.SUPABASE_PROJECT_URL, settings.SUPABASE_API_KEY)
        else:
...
def get_admin_supabase_client() -> Client:
    try:
        if settings.SUPABASE_PROJECT_URL and settings.SUPABASE_SERVICE_ROLE_KEY:
            logger.debug(f"Initializing Admin Supabase client with URL: {settings.SUPABASE_PROJECT_URL}")
